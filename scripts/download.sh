...
cat /script/arealabel.sql | psql $POSTGRES_ARGS

echo "Create lines for labels of natural areas..."
psql $POSTGRES_ARGS -c "DROP VIEW IF EXISTS lowzoom_natural_lines;"
psql $POSTGRES_ARGS -c "DROP VIEW IF EXISTS lowzoom_natural_areas;"
psql $POSTGRES_ARGS -c "$(cat <<QUERY
CREATE OR REPLACE VIEW lowzoom_natural_areas AS
SELECT natural_arealabel(osm_id, way)     as way,
       name,
       areatype,
       way_area
FROM (SELECT osm_id,
             way,
             name,
             (coalesce("natural", "region:type"))              AS areatype,
             way_area
      FROM planet_osm_polygon
      WHERE ("region:type" IN ('natural_area', 'mountain_area') OR
             "natural" IN ('massif', 'mountain_range', 'valley', 'couloir', 'ridge', 'arete', 'gorge', 'canyon'))
        AND name IS NOT NULL) AS natural_areas;
;
QUERY
)"
psql $POSTGRES_ARGS -c "CREATE OR REPLACE VIEW lowzoom_natural_lines AS SELECT way,name,areatype,way_area,(hierarchicregions).nextregionsize AS nextregionsize,(hierarchicregions).subregionsize AS subregionsize FROM (SELECT osm_id,way,name,\"natural\" AS areatype,ST_Length(way)*ST_Length(way)/10 as way_area, OTM_Next_Natural_Area_Size(osm_id,0.0,way) AS hierarchicregions FROM planet_osm_line AS li WHERE \"natural\" IN ('massif', 'mountain_range', 'valley','couloir','ridge','arete','gorge','canyon') AND name IS NOT NULL AND NOT EXISTS (SELECT osm_id FROM planet_osm_polygon AS po WHERE po.osm_id=li.osm_id )) AS natural_lines;"


psql $POSTGRES_ARGS -c "DROP table IF EXISTS naturalarealabels;"
psql $POSTGRES_ARGS -c "CREATE TABLE naturalarealabels (way geometry(LineString,3857), name text, areatype text, way_area real,nextregionsize real,subregionsize real);"
psql $POSTGRES_ARGS -c "INSERT INTO naturalarealabels SELECT * FROM lowzoom_natural_areas;"
psql $POSTGRES_ARGS -c "INSERT INTO naturalarealabels SELECT * FROM lowzoom_natural_lines;"
psql $POSTGRES_ARGS -c "CREATE INDEX naturalarealabels_way_idx ON naturalarealabels USING GIST (way);"
