...
double checkele(char *elestring) {
  /* sanitize ele: substitute "," by ".", ignore "m", convert "ft" to meter,
   * check if ele >> Mount Everest */
  ...
  if (elestring[0] != '\0') {
    ...
    if (textrest[0] != '\0') {
      eleok = 0;
      if ((strcmp(textrest, "m") == 0) || (strcmp(textrest, " m") == 0)) {
        eleok = 1;
      }
      if ((strcmp(textrest, "ft") == 0) || (strcmp(textrest, " ft") == 0)) {
        ele = ele * 0.3048;
        eleok = 1;
      }
    }
    if ((!eleok) || (ele > 9000.0) || (ele < -12000.0)) {
      ele = -32000.0;
    }
  } else {
    ele = -32000.0;
...
double get_height(double lon, double lat, double *adfGeoTransform,
                  GDALRasterBandH hBand, long int xsize, long int ysize) {
  /* get height at lon/lat with elevation data from hBand (parameters in
   * adfGeoTransform) */
  ...
  /* read a 2x2 area from demfile at this pixel, so getting all 4 pixel next to
   * lon/lat  */
  ...
  if (debuglevel > 3) {
    printf("  Getting x/y:%ld %ld at %lf %lf h=%d %d %d %d\n", xpx, ypx,
           adfGeoTransform[0] + xpx * adfGeoTransform[1],
           adfGeoTransform[3] + ypx * adfGeoTransform[5], area2x2[0],
           area2x2[1], area2x2[2], area2x2[3]);
  }
  h = area2x2[0];
  if (area2x2[1] > h) {
    h = area2x2[1];
...
void get_isolation_by_ele(struct list_peak *peak, long int numpeaks,
                          double radius) {
  /* gets a list of peaks, calculate distance of each peak to each other in area
   * of radius, returns nothing */
  ...
  for (i = 0; i < numpeaks; i++) {
    ...
    if (debuglevel > 2) {
      printf("calc dist for peak %lld\n", peak[i].id);
    }
    for (j = i + 1; j < numpeaks; j++) {
      dy = (peak[j].lat - peak[i].lat) * 40000000 / 360;
      if (debuglevel > 3) {
        printf("   compare with other peak %lld\n", peak[j].id);
      }
      if (dy < radius) {
        ...
        if (debuglevel > 3) {
          printf("   compare with other peak %lld, d=%f\n", peak[j].id, d);
        }
        if ((d < peak[j].isolation) && (peak[i].ele > peak[j].ele)) {
          ...
          if (debuglevel > 3) {
            printf("   assign isolation to other peak %lld, d=%f\n", peak[j].id,
                   d);
          }
        }
        if ((d < peak[i].isolation) && (peak[i].ele < peak[j].ele)) {
          ...
          if (debuglevel > 3) {
            printf("   found higher peak %lld for peak %lld, d=%f\n",
...
void get_isolation_by_DEM(struct list_peak *peak, long int numpeaks,
                          double radius, double *adfGeoTransform,
                          GDALDatasetH hDataset, GDALRasterBandH hBand,
  ...
  for (i = 0; i < numpeaks; i++) {
    ...
    /* calc up/le and dw/ri corner of an rectangular area with diameter 2r
     * around each peak */

    le = (long int)floor(
        (peak[i].lon - r * (1 / ccos) * 360 / 40000000 - adfGeoTransform[0]) /
        adfGeoTransform[1]);
    ri = (long int)ceil(
        (peak[i].lon + r * (1 / ccos) * 360 / 40000000 - adfGeoTransform[0]) /
        adfGeoTransform[1]);
    up = (long int)floor(
        (peak[i].lat + r * 360 / 40000000 - adfGeoTransform[3]) /
        adfGeoTransform[5]);
    dw =
        (long int)ceil((peak[i].lat - r * 360 / 40000000 - adfGeoTransform[3]) /
                       adfGeoTransform[5]);
    if (le < 0) {
      le = 0;
    }
    ...
    if (debuglevel > 2) {
      printf("calc isolation for peak %lld iso=%.7lf (%ld %ld points) le=%ld "
             "ri=%ld  dw=%ld up=%ld\n",
    ...
    for (y = up; y < dw; y++) {
      for (x = le; x < ri; x++) {
        if (DEMarea[(x - le) + (y - up) * w] > peak[i].ele + MINDIFF) {
          rx = (peak[i].lon - (adfGeoTransform[0] + adfGeoTransform[1] * x)) *
               40000000 / 360 * ccos;
          ry = (peak[i].lat - (adfGeoTransform[3] + adfGeoTransform[5] * y)) *
               40000000 / 360;
          d = sqrt(rx * rx + ry * ry);
          if (debuglevel > 3) {
            printf(" testing DEM at %ld %ld h=%d d=%f lat=%f lon=%f\n", x - le,
                   y - up, DEMarea[(x - le) + (y - up) * w], d,
                   adfGeoTransform[3] + adfGeoTransform[5] * y,
                   adfGeoTransform[0] + adfGeoTransform[1] * x);
          }
          if ((d > MINISO) && (d < peak[i].isolation)) {
            peak[i].isolation = d;
            peak[i].heigherpoint_lon =
                adfGeoTransform[0] + adfGeoTransform[1] * x;
            peak[i].heigherpoint_lat =
                adfGeoTransform[3] + adfGeoTransform[5] * y;
            if (debuglevel > 3) {
              printf(" found higher DEM at %ld %ld h=%d d=%f lat=%f lon=%f\n",
                     x - le, y - up, DEMarea[(x - le) + (y - up) * w], d,
                     peak[i].heigherpoint_lat, peak[i].heigherpoint_lon);
            }
          }
        }
      }
...
int main(int argc, char *argv[]) {
  ...
  long long int id;
  long int n = 0, i = 0, st_s = 0, st_g = 0, maxnumpeaks = 2000000,
           numpeaks = 0;
  double lon, lat, ele;
  ...
  for (i = 1; i <= argc - 2; i += 2) {
    if (strcmp(argv[i], "-d") == 0) {
      ...
      radius = strtod(argv[i + 1], &textrest);
    } else if (strcmp(argv[i], "-n") == 0) {
      maxnumpeaks = strtod(argv[i + 1], &textrest);
  ...
  if (GDALGetGeoTransform(hDataset, adfGeoTransform) == CE_None) {
    if (debuglevel > 0) {
      printf("%ld Reading %s %dpx x %dpx x %d band\n", time(NULL) - starttime,
             demfile, GDALGetRasterXSize(hDataset),
  ...
  while (fgets(line, 100, stdin)) {
    ...
    if ((ele <= -32000.0)) {
      ele = get_height(lon, lat, adfGeoTransform, hBand,
    ...
    if (id) {
      if (n > maxnumpeaks) {
        ...
        if (debuglevel > 2) {
          printf(" insert: n=%ld id=%lld lon=%f lat=%f ele=%f\n", n, id, lon,
                 lat, ele);
        }
        peak[n].id = id;
        peak[n].lon = lon;
        peak[n].lat = lat;
        peak[n].ele = ele;
        peak[n].isolation = radius;
        peak[n].heigherpoint_lon = peak[n].heigherpoint_lat = 0.0;
        n++;
        numpeaks++;
      }
    }
  ...
  if (debuglevel > 0) {
    printf("%ld Got %ld peaks, %ld without parseable ele\n",
  ...
  if (debuglevel > 0) {
    printf("%ld Got isolation step 1 (peak to peak distance)\n",
           time(NULL) - starttime);
  }

  /* Get isolation as distance to the next higher point in DEM */

  get_isolation_by_DEM(peak, numpeaks, radius, adfGeoTransform, hDataset, hBand,
                       GDALGetRasterXSize(hDataset),
                       GDALGetRasterYSize(hDataset));
  if (debuglevel > 0) {
    printf("%ld Got isolation step 2 (peak to DEM)\n", time(NULL) - starttime);
  }

  for (n = 0; n < numpeaks; n++) {
    if ((!outputformat) || (strcmp(outputformat, "csv") == 0)) {
      if (debuglevel > 0) {
        printf("%lld;%.7lf;%.7lf;%.0lf;%.0lf;%lld;%.7lf;%.7lf\n", peak[n].id,
               peak[n].lon, peak[n].lat, peak[n].ele, peak[n].isolation,
               peak[n].heigherpeak_id, peak[n].heigherpoint_lon,
               peak[n].heigherpoint_lat);
      } else {
        printf("%lld;%.7lf;%.7lf;%.0f\n", peak[n].id, peak[n].lon, peak[n].lat,
               peak[n].isolation);
      }
    } else if (strcmp(outputformat, "sql") == 0) {
      printf("update planet_osm_point set otm_isolation='%.0lf' where "
             "osm_id=%lld;\n",
